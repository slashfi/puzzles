...
Our original internal event handling code is a good example of where this pattern made a huge difference. It started simple, but over time led to giant type unions causing slow type checking, barrel files that imported too much code, and hard-to-trace code. Lastly, it introduced developer friction in maintaining and adding new event handlers.
...
It would be very hard to trace that the wire_sent and wire_created events are triggered from here.
